...
\newpage
\begin{resumo2}
O projeto LibrasVision propõe o desenvolvimento de um sistema de visão computacional capaz de reconhecer sinais dinâmicos da Língua Brasileira de Sinais (Libras) e traduzi-los para texto em tempo real, com o objetivo de reduzir barreiras de comunicação no ambiente acadêmico. A motivação do trabalho fundamenta-se na escassez de Tradutores e Intérpretes de Libras (TILSP) nas instituições de ensino superior brasileiras, o que compromete a plena participação e a autonomia de estudantes surdos. A solução a ser desenvolvida utilizará técnicas de aprendizado profundo, combinando a extração automatizada de marcos anatômicos corporais por meio do framework MediaPipe Holistic com uma rede neural recorrente do tipo LSTM (Long Short-Term Memory), responsável por interpretar a dinâmica temporal dos movimentos. Esta primeira etapa do trabalho, o TCC1, dedica-se ao levantamento bibliográfico, à fundamentação teórica e ao planejamento metodológico do sistema, cuja implementação e avaliação ocorrerão na etapa seguinte. Com base na literatura revisada, projeta-se que o sistema alcance acurácia superior a 85\% nas classes de sinais avaliadas, com latência compatível com uso em tempo real em contextos educacionais, utilizando exclusivamente hardware convencional. Conclui-se que a abordagem baseada em \textit{landmarks} é promissora para o reconhecimento de sinais dinâmicos da Libras em ambiente acadêmico, com potencial de expansão para vocabulários mais amplos e aplicação em diferentes domínios.
...
\newpage
\begin{abstract}
The LibrasVision project proposes the development of a computer vision system capable of recognizing dynamic signs of the Brazilian Sign Language (Libras) and translating them into text in real time, aiming to reduce communication barriers in academic environments. The motivation for this work stems from the shortage of qualified Libras interpreters in Brazilian higher education institutions, which hinders the full participation and autonomy of deaf students. The solution to be developed will employ deep learning techniques, combining automated anatomical landmark extraction through the MediaPipe Holistic framework with a Long Short-Term Memory (LSTM) recurrent neural network responsible for interpreting the temporal dynamics of hand and body movements. This first stage of the work, TCC1, is dedicated to the bibliographic review, theoretical foundation, and methodological planning of the system, whose implementation and evaluation will take place in the following stage. Based on the reviewed literature, the system is projected to achieve accuracy above 85\% across the evaluated sign classes, with latency compatible with real-time use in educational contexts, using only conventional hardware. It is concluded that the landmark-based approach is a promising direction for recognizing dynamic Libras signs in academic environments, with potential for expansion to broader vocabularies and different application domains.
...
% --------------------------------------
\begin{apendicesenv}
\chapter{Protótipos e Configurações de Desenvolvimento}
...
% --------------------------------------
\begin{anexosenv}
\chapter{Protótipos de Código e Planejamento do Dataset}

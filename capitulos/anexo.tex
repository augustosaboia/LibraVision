...
\section{Descrição Planejada do Dataset de Treinamento}

O dataset será coletado seguindo os critérios definidos no planejamento metodológico:

\begin{itemize}
    \item \textbf{Total de sinais previstos:} 150 vídeos (50 por classe)
    \item \textbf{Usuários participantes:} 4 voluntários em ambiente controlado
    ...
    \item \textbf{Variações planejadas:} Diferentes velocidades de execução, ângulos e distâncias da câmera
\end{itemize}

\section{Consentimento dos Participantes}

Este projeto seguirá os protocolos éticos necessários para pesquisa envolvendo seres humanos. Todos os participantes deverão assinar Termo de Consentimento Livre e Esclarecido (TCLE) autorizando o uso de seus vídeos para fins de pesquisa, com garantia de anonimato e privacidade.

Os dados serão armazenados de forma segura e apenas os \textit{landmarks} (coordenadas abstratas) serão utilizados para treinamento, não sendo armazenados os vídeos originais no modelo final.

\section{Requisitos Previstos do Sistema}

Para executar o LibrasVision quando desenvolvido:
